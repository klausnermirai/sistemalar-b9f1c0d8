...
import { useOTAttendances, useCreateOTAttendance } from "@/hooks/useOTAttendances";
...
export function OTAttendances({ residentId }: Props) {
  const { data: attendances = [], isLoading } = useOTAttendances(residentId);
  const create = useCreateOTAttendance();
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast.success("Atendimento de TO registrado");
      setShowForm(false);
      setForm({
        date_time: new Date().toISOString().slice(0, 16),
        activity_type: "",
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Atendimentos de TO</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={form.activity_type} onValueChange={(v) => setForm({ ...form, activity_type: v })}>
                  ...
                  <SelectContent>
                    <SelectItem value="avd_treino">Treino de AVDs</SelectItem>
                    <SelectItem value="estimulacao_cognitiva">Estimulação Cognitiva</SelectItem>
                    <SelectItem value="adaptacao_ambiental">Adaptação Ambiental</SelectItem>
                    <SelectItem value="atividade_terapeutica">Atividade Terapêutica</SelectItem>
                    <SelectItem value="orientacao_equipe">Orientação à Equipe</SelectItem>
